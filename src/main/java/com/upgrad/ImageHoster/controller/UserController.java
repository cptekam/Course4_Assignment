...
public class UserController {
    ...
    public String signUpUser(@RequestParam("username") String username,
        ...
        if (password.length() >= 6 && username.length() >= 6) {
            ...
            User existingUser = userService.getByName(user.getUsername().trim());

            //Check if a new user is trying to register with a username that has been previously registered
            if (existingUser == null || !user.getUsername().equals(existingUser.getUsername())) {
                ...
                // iflash an error on the user registration page saying that the username has been previously registered.
            ...
            if (username.length() < 6) {
                String error = "username and password needs to be 6 characters or longer";
